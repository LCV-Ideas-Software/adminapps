...
| Release | Scope |
|---|---|
| **`v02.00.12`** | **README organizational standardization.** Adopted the shared repository README opening pattern, corrected public release and clone links to the organization, surfaced the top-level version-history table, and kept the GitHub Sponsors link on `lcv-leo` by explicit beneficiary decision. |
| **`v02.00.11`** | **Pages modernization.** Migrated fully to the current GitHub Pages artifact-deployment model and enabled idempotent Pages setup for fresh clones/forks. |
| **`v02.00.10`** | **Cloudflare Pages config fix.** Removed unsupported `observability` config from the Pages-side `wrangler.json` after runtime validation. |
| **`v02.00.09`** | **Pre-modernization hardening.** Finalized the baseline immediately before the Pages and publication modernization sequence. |

## What it does

`adminapps` is a hub of small administrative React modules, each rendered as a card in a configurable grid. Cards point to compliance / configuration surfaces — a license panel (`LicencasModule`), a compliance banner, third-party attribution. The hub itself is a static Cloudflare Pages app; per-card backends (when needed) live in `functions/api/*` as Cloudflare Pages Functions backed by a D1 binding (`BIGDATA_DB`).

The card list is data-driven: `public/cards.json` declares title/description/href/icon for each card. Adding a card means appending a row there + (optionally) creating a Pages Function endpoint.
...
Browser -> Cloudflare Pages (static React build)
                   |
                   v
         <Card> components in src/modules/*
                   |
       (client-side fetch to /api/* on same origin)
                   |
                   v
...
```bash
git clone https://github.com/LCV-Ideas-Software/adminapps.git
cd adminapps
npm ci
```
...
```bash
npm run build
npx wrangler pages deploy dist --project-name=adminapps
```
...
Edit `public/cards.json` to declare your own cards. Each card is `{ title, description, href, icon }`.
