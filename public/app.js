...
// Versão: v01.03.00
// Descrição: Catálogo administrativo — carrega cards de admin-app (bigdata_db) com fallback para cards.json local.

const SAFE_PROTOCOLS = new Set(["https:"]);
const APP_VERSION = 'APP v01.03.00';
const ADMIN_BASE_URL = 'https://admin.lcv.app.br';
const CONFIG_ENDPOINT = `${ADMIN_BASE_URL}/api/adminhub/config`;
...
function createCard(app) {
    ...
    card.dataset.level = "restricted";
    card.href = app.url;
    card.target = "_blank";
    card.rel = "noopener noreferrer";
    card.setAttribute("aria-label", `${app.name} (acesso restrito)`);
...
function mountCards(cards) {
    const root = document.getElementById("cards-admin");
