 ...
 */
/* ====================================================================
   AdminHub — App Component (React 19 + TypeScript)
...
const APP_VERSION = 'APP v02.00.09';
...
function Card({ card }: { card: CardData }) {
  ...
  return (
    /* biome-ignore lint/a11y/useSemanticElements: hiding the browser status bar on hover is an intentional UX requirement for these cards. */
    <div
      ...
      aria-label={`${card.name} (acesso restrito)`}
    ...
    <>
      ...
      {!showLicenses ? (
        <main className="layout" id="app" aria-live="polite">
          ...
          <section className="catalog" aria-labelledby="section-admin-title">
            <h2 id="section-admin-title">
              <span className="status-dot restricted" />
              Ferramentas &amp; Gestão
            </h2>
            <div className="card-grid" id="cards-admin">
              {error ? <p>{error}</p> : cards.filter((c) => isSafeUrl(c.url)).map((c) => <Card key={c.url} card={c} />)}
            </div>
          </section>
      ...
      <footer className="app-footer md3-surface">
        <p>&copy; {new Date().getFullYear()} Painel Administrativo | Leonardo Cardozo Vargas</p>
        <p className="app-version" title="Versionamento Semântico">
          {APP_VERSION}
        </p>
      </footer>
